...
/// ```
pub fn solve_givens<T: Float, const D: usize, const S: usize>(
    sup: &[T; S],
    diag: &[T; D],
    sub: &[T; S],
    rhs: &[T; D],
) -> Result<[T; D], SolverErrors> {
    const {
        assert!(
            D == S + 1,
            "Sub and sup diagonals must be exctly 1 element smaller than main diagonal"
        )
    };
...
///
/// ```
pub fn precompute_givens<T: Float, const D: usize, const S: usize>(
    sup: &[T; S],
    diag: &[T; D],
    sub: &[T; S],
) -> Result<TridiagSysPrecomp<T, D, S>, SolverErrors> {
    const {
        assert!(
            D == S + 1,
            "Sub and sup diagonals must be exctly 1 element smaller than main diagonal"
    ...
    Ok(TridiagSysPrecomp {
        ...
        sins_cosins: if sins_cosins.len() > 0 {
            Some(sins_cosins)
        } else {
            None
        },
...
pub fn precompute_givens_ruiz<T: Float, const D: usize, const S: usize>(
    ...
    get_ruiz_equilibrium_mul(
        &diag_b, &sup_b, &sub_b, &mut row_b, &mut col_b, r_iter, r_eps,
    )?;
    apply_ruiz(&mut diag_b, &mut sup_b, &mut sub_b, &row_b, &col_b);
...
impl<T: Float, const D: usize, const S: usize> TridiagSysRuizPrecomp<T, D, S> {
    ...
    pub fn solve_givens_rhs(&self, rhs: &[T; D]) -> Result<[T; D], SolverErrors> {
        ...
        rhsl.iter_mut()
            .zip(self.row_mul)
            .for_each(|(r, row)| *r = *r * row);

        let mut x = self.sys.solve_givens_rhs(&rhsl)?;
        x.iter_mut()
            .zip(self.col_mul)
            .for_each(|(x, col)| *x = *x * col);
...
/// ```
///
pub fn tridiag_iter_kaczmarz<T: Float, const D: usize, const S: usize>(
    sup: &[T; S],
    diag: &[T; D],
    sub: &[T; S],
    rhs: &[T; D],
...
pub fn compute_ruiz_scaling<T: Float, const D: usize, const S: usize>(
    ...
    let mut row_b = [T::one(); D];
    let mut col_b = [T::one(); D];
    get_ruiz_equilibrium_mul(diag, sup, sub, &mut row_b, &mut col_b, iter, eps)?;
    Ok((row_b, col_b))
}
...
/// ```
///
pub fn solve_givens_ruiz_precond<T: Float, const D: usize, const S: usize>(
    sup: &[T; S],
    diag: &[T; D],
    sub: &[T; S],
    rhs: &[T; D],
