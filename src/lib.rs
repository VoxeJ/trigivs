...
/// ```
pub fn compute_tridiag_determinant<T: Float>(
    sup: &[T],
    diag: &[T],
    sub: &[T],
) -> Result<T, SolverErrors> {
...
/// ```
///
pub fn compute_solution_residual_norm<T: Float>(
    sup: &[T],
    diag: &[T],
    sub: &[T],
    rhs: &[T],
    x: &[T],
) -> Result<T, solver_error::SolverErrors> {
    ...
    let mut sum = (rhs[0] - diag[0] * x[0] - sup[0] * x[1]).powi(2)
